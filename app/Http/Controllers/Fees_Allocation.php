...
class Fees_Allocation extends Controller
{
    ...
    // {
    //         $classes = DB::table('classes')->select('class_name', 'section_name')->get();
    // dd($classes);



    // ->update(['feesallocations.class' => DB::raw('classes.class_name')]);


    // }

    // public function showAddFeeAllocationForm()
    // {

    //     $feeall = Fees::all();

    //     $classNames = DB::table('classes')->pluck('class_name')->toArray();
    //     return view('addfeeallocation', ['classNames' => $classNames], compact('feeallocation'));
    // }
